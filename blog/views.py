...
from django.contrib.auth.decorators import permission_required, login_required # Restriction d'accés à la page
...
def blog_and_photo_upload(request):
    ...
    if request.method == 'POST':
        ...
        photo_form = forms.PhotoForm(request.POST, request.FILES)
            ...
            # équivaux à: if blog_form.is_valid() or photo_form.is_valid():
        if all([blog_form.is_valid(), photo_form.is_valid()]):
            # équivaux à: if blog_form.is_valid() and photo_form.is_valid():
...
def create_multiple_photos(request):
    PhotoFormSet = formset_factory(forms.PhotoForm, extra=10)
...
def photo_view(request, photo_id):
    ...
    return render(request, 'blog/photo_snippet.html', {'photo': photo})
...
def home(request):
    blogs = models.Blog.objects.filter(
        ...
        Q(starred=True) # equivalent ~Q(starred=False)  ~:not
    )
    photos = models.Photo.objects.filter(
